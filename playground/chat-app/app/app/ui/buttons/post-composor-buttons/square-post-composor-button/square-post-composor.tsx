...
export default function SquarePostComposorButton({ reverse, delay, src, title, alt, node, styleSheetNode }: input) {
    ...
    // Adjusted widths to dynamically scale beautifully if a long text layout is provided
    const bw = src && title ? 'auto' : square(styleSheetNode?.btnWidth ?? 20)
    const bh = square(styleSheetNode?.btnWidth ?? 20)
