...
// PlayPIERUMBasicExample demonstrates using the SDK with GoMonitor and the client for testing
func PlayPIERUMBasicExample() {
	...
	rumServer.GoMonitor(ctx, func(result *rumcore.IResults) {
		...
		func() {
			...
			sendSDKRequest(serverAddr, profileName, Req{
				ID:      &id,
				Name:    &name,
				Query:   &query,
				Profile: profileName,
...
func sendSDKRequest(addr, profileName string, req Req) error {
